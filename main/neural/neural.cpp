...
NeuralNet::NeuralNet()
{
    ...
    {
        ESP_LOGE(TAG, "Model versiyası uyğun gəlmir: %d", model->version());
        return;
    }
    ...
    {
        ESP_LOGE(TAG, "FullyConnected operatoru əlavə edilə bilmədi!");
        return;
    }
    // Lazım olan operatorları əlavə edirik (burada FullyConnected)
    if (resolver.AddSoftmax() != kTfLiteOk)
    {
        ESP_LOGE(TAG, "Softmax operatoru əlavə edilə bilmədi!");
        return;
    }
    // Tensörləri ayırırıq
    if (interpreter.AllocateTensors() != kTfLiteOk)
    {
        ESP_LOGE(TAG, "Tensörlərin ayrılması uğursuz oldu!");
        return;
    }
}
int NeuralNet::getData(float data[], bool quant_status)
{
    ...
    {
        ESP_LOGE(TAG, "Model çalıştırılamadı!");
        return -1;
    }
